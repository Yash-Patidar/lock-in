...
export default function SocialLinks() {
  const socialLinks = [
    ...
    {
      name: 'Reddit',
      url: 'https://reddit.com/u/yash__patidar_',
      icon: (
        <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="fixed top-4 right-4 md:top-6 md:right-6 z-40">
      <div className="flex items-center gap-1 bg-gray-900/80 backdrop-blur-md border border-gray-700/50 rounded-full p-1.5 shadow-lg shadow-black/25">
        {socialLinks.map((link, index) => (
          <a
            ...
            className={`w-8 h-8 flex items-center justify-center rounded-full text-gray-400 hover:text-white transition-all duration-200 hover:scale-110 group relative ${
              index === 0 ? 'hover:bg-blue-500/20 hover:text-blue-400' :
              index === 1 ? 'hover:bg-gray-500/20 hover:text-gray-300' :
        ...
        {/* GitHub Repo Link with special styling */}
        <a
          href="https://github.com/Yash-Patidar/lock-in"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-1 px-2 py-1 text-xs text-gray-400 hover:text-green-400 transition-colors duration-200 group relative"
